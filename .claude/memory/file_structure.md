...
/internal/
  ...
  /plugin/                # NEW 2026-07-25: package plugin, extracted from internal/app (files were prefixed plugin_*.go there) to avoid coupling plugin lifecycle code to the App god-object; internal/app now imports this package and qualifies all references as plugin.PluginLoader/plugin.PluginStatus*/plugin.NewPluginLoader/plugin.NewPluginAssetHandler
    ...
    # /pb/ REMOVED 2026-08-18 (plugin-architecture-inversion Phase 4 cleanup): the generic `Plugin` gRPC contract (plugin.proto/plugin.pb.go/plugin_grpc.pb.go — GetCapabilities, SetClusterContext, Invoke(method, payloadJson)->(payloadJson, error)) now lives at packages/core/pb, imported by internal/plugin as a Go module dependency instead of a local copy. Every plugin (Helm included) is still reached ONLY through this generic interface — internal/ and internal/app must never import a concrete plugin's own package or proto, see [[gotcha_generic_plugin_grpc_boundary]]
