...
/**
 * useMutateRemovePlugin triggers plugin removal via a mutation:
 * - Takes pluginId as a **call-time** argument (not hook-time)
 * - This allows one hook instance to serve every row in a list
 * - Wraps RemovePlugin Wails binding and invalidates the same queries as useRemovePlugin.ts
 * - Callers can still catch errors and toast
 */
export const useMutateRemovePlugin = () => {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: async ({ pluginId }: RemoveMutationVariables) => {
