...
import { useNamespacesUpdateEvents } from "../useNamespacesUpdateEvents";
...
describe("useNamespacesUpdateEvents", () => {
  it("returns an empty array initially", () => {
    const { result } = renderHook(() => useNamespacesUpdateEvents());
  ...
  it("updates with pushed data when event is triggered", () => {
    ...
    act(() => triggerEvent("namespaces:update", payload));
  ...
  it("clears the previous data when a new event is triggered", () => {
    ...
    act(() => triggerEvent("namespaces:update", payload1));
    expect(result.current).toEqual(payload1);

    const payload2 = [{ Name: "two" }];
    act(() => triggerEvent("namespaces:update", payload2));
