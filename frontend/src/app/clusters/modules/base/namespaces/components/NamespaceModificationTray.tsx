...
import { useGetNamespaceYAML } from "../hooks/data-access/useGetNamespaceYAML";
import { useUpdateNamespaceYAML } from "../hooks/data-mutation/useUpdateNamespaceYAML";
...
export const NamespaceModificationTray: FC<ModificationTrayContentProps> = ({
  tab,
  collapsed,
  onClose,
}) => {
  const { activeContext } = useMainLayoutContext();
  const namespaceName = tab.name;
  ...
  } = useGetNamespaceYAML(activeContext, namespaceName);
  ...
  const { mutate: updateYAML, isPending: isUpdating } = useUpdateNamespaceYAML();
    ...
    <>
      ...
      >
        {isLoading ? (
          ...
          <p className="text-muted-foreground p-4 text-xs">No YAML available for this Namespace.</p>
        ) : (
          <Textarea
            variant="yaml"
            borderRounded={false}
