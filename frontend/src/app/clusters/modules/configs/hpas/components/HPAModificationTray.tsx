...
export const HPAModificationTray: FC<ModificationTrayContentProps> = ({
  tab,
  collapsed,
  onClose,
}) => {
  ...
  } = useGetHPAYAML(activeContext, namespace, hpaName);
  ...
  const { mutate: updateYAML, isPending: isUpdating } = useUpdateHPAYAML();
    ...
    <>
      ...
      >
        {isLoading ? (
          ...
          <p className="text-muted-foreground p-4 text-xs">No YAML available for this HPA.</p>
        ) : (
          <Textarea
            variant="yaml"
            borderRounded={false}
