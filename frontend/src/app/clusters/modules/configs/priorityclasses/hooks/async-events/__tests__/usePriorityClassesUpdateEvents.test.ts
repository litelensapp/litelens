...
import { usePriorityClassesUpdateEvents } from "../usePriorityClassesUpdateEvents";
...
describe("usePriorityClassesUpdateEvents", () => {
  it("returns an empty array initially", () => {
    const { result } = renderHook(() => usePriorityClassesUpdateEvents());
  ...
  it("updates with pushed data when event is triggered", () => {
    ...
    act(() => triggerEvent("priorityclasses:update", payload));
  ...
  it("clears the previous data when a new event is triggered", () => {
    ...
    act(() => triggerEvent("priorityclasses:update", payload1));
    expect(result.current).toEqual(payload1);

    const payload2 = [{ Name: "two" }];
    act(() => triggerEvent("priorityclasses:update", payload2));
