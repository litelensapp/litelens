...
import { useGetResourceQuotaYAML } from "../hooks/data-access/useGetResourceQuotaYAML";
import { useUpdateResourceQuotaYAML } from "../hooks/data-mutation/useUpdateResourceQuotaYAML";
...
export const ResourceQuotaModificationTray: FC<ModificationTrayContentProps> = ({
  tab,
  collapsed,
  onClose,
}) => {
  ...
  const resourceQuotaName = tab.name;
  ...
  } = useGetResourceQuotaYAML(activeContext, namespace, resourceQuotaName);

  const { mutate: updateYAML, isPending: isUpdating } = useUpdateResourceQuotaYAML();
