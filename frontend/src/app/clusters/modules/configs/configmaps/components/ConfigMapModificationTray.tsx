...
import { useGetConfigMapYAML } from "../hooks/data-access/useGetConfigMapYAML";
import { useUpdateConfigMapYAML } from "../hooks/data-mutation/useUpdateConfigMapYAML";
...
export const ConfigMapModificationTray: FC<ModificationTrayContentProps> = ({
  tab,
  collapsed,
  onClose,
}) => {
  ...
  } = useGetConfigMapYAML(activeContext, namespace, configMapName);

  const { mutate: updateYAML, isPending: isUpdating } = useUpdateConfigMapYAML();
  ...
  if (!tab.namespace) {
    return (
      <p className="p-4 text-xs text-destructive">
        ConfigMap namespace is required but not provided.
      </p>
    );
  }
    ...
    <>
      ...
      >
        {isLoading ? (
          ...
          <p className="p-4 text-xs text-muted-foreground">No YAML available for this ConfigMap.</p>
        ) : (
          <Textarea
            variant="yaml"
            borderRounded={false}
