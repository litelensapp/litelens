...
interface ConfigMapDeleteConfirmationModalProps {
  open: boolean;
  mode: "single" | "bulk";
  name?: string;
  namespace?: string;
...
export const ConfigMapDeleteConfirmationModal: FC<ConfigMapDeleteConfirmationModalProps> = ({
  open,
  mode,
  name,
  namespace,
  ...
  const { title, description } = useMemo(() => {
    ...
    const titleText = `Delete ${count} ConfigMap${count === 1 ? "" : "s"}`;
    ...
    const descriptionElement = (() => {
      if (namespaces.length === 1) {
        ...
        return (
          <>
            This will permanently delete {count} configmap{count === 1 ? "" : "s"} from namespace{" "}
      ...
      return (
        <>
          This will permanently delete {count} configmaps from {namespaces.length} namespaces. This
