import { useGetLeaseYAML } from "../hooks/data-access/useGetLeaseYAML";
import { useUpdateLeaseYAML } from "../hooks/data-mutation/useUpdateLeaseYAML";
...
export const LeaseModificationTray: FC<ModificationTrayContentProps> = ({
  tab,
  collapsed,
  onClose,
}) => {
  ...
  } = useGetLeaseYAML(activeContext, namespace, leaseName);

  const { mutate: updateYAML, isPending: isUpdating } = useUpdateLeaseYAML();
  ...
  if (!tab.namespace) {
    return (
      <p className="p-4 text-xs text-destructive">Lease namespace is required but not provided.</p>
    );
  }
    ...
    <>
      ...
      >
        {isLoading ? (
          ...
          <p className="p-4 text-xs text-muted-foreground">No YAML available for this Lease.</p>
        ) : (
          <Textarea
            variant="yaml"
            borderRounded={false}
