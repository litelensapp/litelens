...
interface SecretDeleteConfirmationModalProps {
  open: boolean;
  mode: "single" | "bulk";
  name?: string;
  namespace?: string;
...
export const SecretDeleteConfirmationModal: FC<SecretDeleteConfirmationModalProps> = ({
  open,
  mode,
  name,
  namespace,
  ...
  const { title, description } = useMemo(() => {
    ...
    const titleText = `Delete ${count} Secret${count === 1 ? "" : "s"}`;
    ...
    const descriptionElement = (() => {
      ...
      return (
        <>
          This will permanently delete {count} secrets from {namespaces.length} namespaces. This
