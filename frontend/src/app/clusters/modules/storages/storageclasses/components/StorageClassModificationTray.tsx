...
export const StorageClassModificationTray: FC<ModificationTrayContentProps> = ({
  tab,
  collapsed,
  onClose,
}) => {
  const { activeContext } = useMainLayoutContext();
  const scName = tab.name;
  ...
  } = useGetStorageClassYAML(activeContext, scName);
  ...
  const { mutate: updateYAML, isPending: isUpdating } = useUpdateStorageClassYAML();
