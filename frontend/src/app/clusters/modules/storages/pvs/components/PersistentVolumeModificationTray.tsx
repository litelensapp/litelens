...
export const PersistentVolumeModificationTray: FC<ModificationTrayContentProps> = ({
  tab,
  collapsed,
  onClose,
}) => {
  const { activeContext } = useMainLayoutContext();
  const pvName = tab.name;
  ...
  } = useGetPersistentVolumeYAML(activeContext, pvName);
  ...
  const { mutate: updateYAML, isPending: isUpdating } = useUpdatePersistentVolumeYAML();
