...
import { usePersistentVolumesUpdateEvents } from "../usePersistentVolumesUpdateEvents";
...
describe("usePersistentVolumesUpdateEvents", () => {
  it("returns an empty array initially", () => {
    const { result } = renderHook(() => usePersistentVolumesUpdateEvents());
  ...
  it("updates with pushed data when event is triggered", () => {
    ...
    act(() => triggerEvent("pvs:update", payload));
  ...
  it("clears the previous data when a new event is triggered", () => {
    ...
    act(() => triggerEvent("pvs:update", payload1));
    expect(result.current).toEqual(payload1);

    const payload2 = [{ Name: "two" }];
    act(() => triggerEvent("pvs:update", payload2));
