...
interface EndpointDeleteConfirmationModalProps {
  open: boolean;
  mode: "single" | "bulk";
  name?: string;
  namespace?: string;
...
export const EndpointDeleteConfirmationModal: FC<EndpointDeleteConfirmationModalProps> = ({
  open,
  mode,
  name,
  namespace,
  ...
  const { title, description } = useMemo(() => {
    ...
    const titleText = `Delete ${count} Endpoint${count === 1 ? "" : "s"}`;
    ...
    const descriptionElement = (() => {
      if (namespaces.length === 1) {
        ...
        return (
          <>
            This will permanently delete {count} endpoint{count === 1 ? "" : "s"} from namespace{" "}
      ...
      return (
        <>
          This will permanently delete {count} endpoints from {namespaces.length} namespaces. This
