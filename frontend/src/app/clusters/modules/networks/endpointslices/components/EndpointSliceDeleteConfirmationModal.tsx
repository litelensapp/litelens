...
interface EndpointSliceDeleteConfirmationModalProps {
  open: boolean;
  mode: "single" | "bulk";
  name?: string;
  namespace?: string;
...
export const EndpointSliceDeleteConfirmationModal: FC<
  EndpointSliceDeleteConfirmationModalProps
> = ({ open, mode, name, namespace, items, isPending, onClose, onConfirm }) => {
  ...
  const { title, description } = useMemo(() => {
    ...
    const titleText = `Delete ${count} EndpointSlice${count === 1 ? "" : "s"}`;
    ...
    const descriptionElement = (() => {
      ...
      return (
        <>
          This will permanently delete {count} endpointslices from {namespaces.length} namespaces.
