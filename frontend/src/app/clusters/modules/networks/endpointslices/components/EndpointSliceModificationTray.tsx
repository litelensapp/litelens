...
import { useGetEndpointSliceYAML } from "../hooks/data-access/useGetEndpointSliceYAML";
import { useUpdateEndpointSliceYAML } from "../hooks/data-mutation/useUpdateEndpointSliceYAML";
...
export const EndpointSliceModificationTray: FC<ModificationTrayContentProps> = ({
  tab,
  collapsed,
  onClose,
}) => {
  ...
  const endpointSliceName = tab.name;
  ...
  } = useGetEndpointSliceYAML(activeContext, namespace, endpointSliceName);

  const { mutate: updateYAML, isPending: isUpdating } = useUpdateEndpointSliceYAML();
  ...
  if (!tab.namespace) {
    return (
      <p className="p-4 text-xs text-destructive">
        EndpointSlice namespace is required but not provided.
      </p>
    );
  }
      ...
      >
        {isLoading ? (
          ...
          <p className="p-4 text-xs text-muted-foreground">
            No YAML available for this EndpointSlice.
          </p>
        ) : (
          <Textarea
            variant="yaml"
