...
interface IngressDeleteConfirmationModalProps {
  open: boolean;
  mode: "single" | "bulk";
  name?: string;
  namespace?: string;
...
export const IngressDeleteConfirmationModal: FC<IngressDeleteConfirmationModalProps> = ({
  open,
  mode,
  name,
  namespace,
  ...
  const { title, description } = useMemo(() => {
    ...
    const titleText = `Delete ${count} Ingress${count === 1 ? "" : "es"}`;
    ...
    const descriptionElement = (() => {
      if (namespaces.length === 1) {
        ...
        return (
          <>
            This will permanently delete {count} ingress{count === 1 ? "" : "es"} from namespace{" "}
      ...
      return (
        <>
          This will permanently delete {count} ingresses from {namespaces.length} namespaces. This
