...
import { useGetIngressYAML } from "../hooks/data-access/useGetIngressYAML";
import { useUpdateIngressYAML } from "../hooks/data-mutation/useUpdateIngressYAML";
...
export const IngressModificationTray: FC<ModificationTrayContentProps> = ({
  tab,
  collapsed,
  onClose,
}) => {
  ...
  } = useGetIngressYAML(activeContext, namespace, ingressName);

  const { mutate: updateYAML, isPending: isUpdating } = useUpdateIngressYAML();
  ...
  if (!tab.namespace) {
    return (
      <p className="text-destructive p-4 text-xs">
        Ingress namespace is required but not provided.
      </p>
    );
  }
    ...
    <>
      ...
      >
        {isLoading ? (
          ...
          <p className="text-muted-foreground p-4 text-xs">No YAML available for this Ingress.</p>
        ) : (
          <Textarea
            variant="yaml"
            borderRounded={false}
