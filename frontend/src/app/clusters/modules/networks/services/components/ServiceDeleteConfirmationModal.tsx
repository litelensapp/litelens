...
export const ServiceDeleteConfirmationModal: FC<ServiceDeleteConfirmationModalProps> = ({
  open,
  mode,
  name,
  namespace,
  ...
  const { title, description } = useMemo(() => {
    ...
    const titleText = `Delete ${count} Service${count === 1 ? "" : "s"}`;
    ...
    const descriptionElement = (() => {
      if (namespaces.length === 1) {
        ...
        return (
          <>
            This will permanently delete {count} service{count === 1 ? "" : "s"} from namespace{" "}
      ...
      return (
        <>
          This will permanently delete {count} services from {namespaces.length} namespaces. This
