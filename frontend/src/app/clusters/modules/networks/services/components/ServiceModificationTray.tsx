import { useGetServiceYAML } from "../hooks/data-access/useGetServiceYAML";
import { useUpdateServiceYAML } from "../hooks/data-mutation/useUpdateServiceYAML";
...
export const ServiceModificationTray: FC<ModificationTrayContentProps> = ({
  tab,
  collapsed,
  onClose,
}) => {
  ...
  } = useGetServiceYAML(activeContext, namespace, serviceName);

  const { mutate: updateYAML, isPending: isUpdating } = useUpdateServiceYAML();
  ...
  if (!tab.namespace) {
    return (
      <p className="p-4 text-xs text-destructive">
        Service namespace is required but not provided.
      </p>
    );
  }
    ...
    <>
      ...
      >
        {isLoading ? (
          ...
          <p className="p-4 text-xs text-muted-foreground">No YAML available for this Service.</p>
        ) : (
          <Textarea
            variant="yaml"
            borderRounded={false}
