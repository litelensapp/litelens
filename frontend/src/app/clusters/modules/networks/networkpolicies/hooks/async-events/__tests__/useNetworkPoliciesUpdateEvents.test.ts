...
import { useNetworkPoliciesUpdateEvents } from "../useNetworkPoliciesUpdateEvents";
...
describe("useNetworkPoliciesUpdateEvents", () => {
  it("returns an empty array initially", () => {
    const { result } = renderHook(() => useNetworkPoliciesUpdateEvents());
  ...
  it("updates with pushed data when event is triggered", () => {
    ...
    act(() => triggerEvent("networkpolicies:update", payload));
  ...
  it("clears the previous data when a new event is triggered", () => {
    ...
    act(() => triggerEvent("networkpolicies:update", payload1));
    expect(result.current).toEqual(payload1);

    const payload2 = [{ Name: "two" }];
    act(() => triggerEvent("networkpolicies:update", payload2));
