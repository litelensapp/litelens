...
export const ServiceAccountModificationTray: FC<ModificationTrayContentProps> = ({
  tab,
  collapsed,
  onClose,
}) => {
  ...
  const serviceAccountName = tab.name;
  ...
  } = useGetServiceAccountYAML(activeContext, namespace, serviceAccountName);

  const { mutate: updateYAML, isPending: isUpdating } = useUpdateServiceAccountYAML();
