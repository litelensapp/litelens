...
import { useRoleBindingsUpdateEvents } from "../useRoleBindingsUpdateEvents";
...
describe("useRoleBindingsUpdateEvents", () => {
  it("returns an empty array initially", () => {
    const { result } = renderHook(() => useRoleBindingsUpdateEvents());
  ...
  it("updates with pushed data when event is triggered", () => {
    ...
    act(() => triggerEvent("rolebindings:update", payload));
  ...
  it("clears the previous data when a new event is triggered", () => {
    ...
    act(() => triggerEvent("rolebindings:update", payload1));
    expect(result.current).toEqual(payload1);

    const payload2 = [{ Name: "two" }];
    act(() => triggerEvent("rolebindings:update", payload2));
