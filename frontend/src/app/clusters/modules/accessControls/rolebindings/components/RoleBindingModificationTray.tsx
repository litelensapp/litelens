...
export const RoleBindingModificationTray: FC<ModificationTrayContentProps> = ({
  tab,
  collapsed,
  onClose,
}) => {
  ...
  const roleBindingName = tab.name;
  ...
  } = useGetRoleBindingYAML(activeContext, namespace, roleBindingName);

  const { mutate: updateYAML, isPending: isUpdating } = useUpdateRoleBindingYAML();
