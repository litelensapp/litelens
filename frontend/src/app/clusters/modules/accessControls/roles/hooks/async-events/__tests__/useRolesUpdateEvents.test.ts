...
import { useRolesUpdateEvents } from "../useRolesUpdateEvents";
...
describe("useRolesUpdateEvents", () => {
  it("returns an empty array initially", () => {
    const { result } = renderHook(() => useRolesUpdateEvents());
  ...
  it("updates with pushed data when event is triggered", () => {
    ...
    act(() => triggerEvent("roles:update", payload));
  ...
  it("clears the previous data when a new event is triggered", () => {
    ...
    act(() => triggerEvent("roles:update", payload1));
    expect(result.current).toEqual(payload1);

    const payload2 = [{ Name: "two" }];
    act(() => triggerEvent("roles:update", payload2));
