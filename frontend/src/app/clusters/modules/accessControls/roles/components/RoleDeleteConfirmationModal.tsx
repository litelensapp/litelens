...
export const RoleDeleteConfirmationModal: FC<RoleDeleteConfirmationModalProps> = ({
  open,
  mode,
  name,
  namespace,
  ...
  const { title, description } = useMemo(() => {
    ...
    const titleText = `Delete ${count} Role${count === 1 ? "" : "s"}`;
    ...
    const descriptionElement = (() => {
      ...
      return (
        <>
          This will permanently delete {count} roles from {namespaces.length} namespaces. This
