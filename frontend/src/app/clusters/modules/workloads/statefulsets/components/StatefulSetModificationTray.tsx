import { useGetStatefulSetYAML } from "../hooks/data-access/useGetStatefulSetYAML";
import { useUpdateStatefulSetYAML } from "../hooks/data-mutation/useUpdateStatefulSetYAML";
...
export const StatefulSetModificationTray: FC<ModificationTrayContentProps> = ({
  tab,
  collapsed,
  onClose,
}) => {
  const namespace = tab.namespace || "";
  const statefulSetName = tab.name;
  ...
  } = useGetStatefulSetYAML(activeContext, namespace, statefulSetName);

  const { mutate: updateYAML, isPending: isUpdating } = useUpdateStatefulSetYAML();
  ...
  if (!tab.namespace) {
    return (
      <p className="text-destructive p-4 text-xs">
        StatefulSet namespace is required but not provided.
      </p>
    );
  }
      ...
      >
        {isLoading ? (
          ...
          <p className="text-muted-foreground p-4 text-xs">
            No YAML available for this StatefulSet.
          </p>
        ) : (
          <Textarea
            variant="yaml"
