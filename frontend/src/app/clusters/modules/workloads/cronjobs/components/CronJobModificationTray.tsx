import { useGetCronJobYAML } from "../hooks/data-access/useGetCronJobYAML";
import { useUpdateCronJobYAML } from "../hooks/data-mutation/useUpdateCronJobYAML";
...
export const CronJobModificationTray: FC<ModificationTrayContentProps> = ({
  tab,
  collapsed,
  onClose,
}) => {
  const namespace = tab.namespace || "";
  const cronJobName = tab.name;
  ...
  } = useGetCronJobYAML(activeContext, namespace, cronJobName);

  const { mutate: updateYAML, isPending: isUpdating } = useUpdateCronJobYAML();
  ...
  if (!tab.namespace) {
    return (
      <p className="text-destructive p-4 text-xs">
        CronJob namespace is required but not provided.
      </p>
    );
  }
    ...
    <>
      ...
      >
        {isLoading ? (
          ...
          <p className="text-muted-foreground p-4 text-xs">No YAML available for this CronJob.</p>
        ) : (
          <Textarea
            variant="yaml"
            borderRounded={false}
