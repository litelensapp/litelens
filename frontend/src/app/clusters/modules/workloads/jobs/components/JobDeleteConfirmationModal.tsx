...
interface JobDeleteConfirmationModalProps {
  open: boolean;
  mode: "single" | "bulk";
  name?: string;
  namespace?: string;
...
export const JobDeleteConfirmationModal: FC<JobDeleteConfirmationModalProps> = ({
  open,
  mode,
  name,
  namespace,
  ...
  const { title, description } = useMemo(() => {
    if (mode === "single") {
      return {
        title: (
          <>
            Delete Job: <span className="text-muted-foreground font-mono font-normal">{name}</span>
          </>
        ),
        description: (
          <>
    ...
    const titleText = `Delete ${count} Job${count === 1 ? "" : "s"}`;
    ...
    const descriptionElement = (() => {
      if (namespaces.length === 1) {
        ...
        return (
          <>
            This will permanently delete {count} job{count === 1 ? "" : "s"} from namespace{" "}
      ...
      return (
        <>
          This will permanently delete {count} jobs from {namespaces.length} namespaces. This action
          cannot be undone.
        </>
      );
    })();
