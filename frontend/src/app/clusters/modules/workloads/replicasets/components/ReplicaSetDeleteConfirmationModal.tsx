...
interface ReplicaSetDeleteConfirmationModalProps {
  open: boolean;
  mode: "single" | "bulk";
  name?: string;
  namespace?: string;
...
export const ReplicaSetDeleteConfirmationModal: FC<ReplicaSetDeleteConfirmationModalProps> = ({
  open,
  mode,
  name,
  namespace,
  ...
  const { title, description } = useMemo(() => {
    ...
    const titleText = `Delete ${count} ReplicaSet${count === 1 ? "" : "s"}`;
    ...
    const descriptionElement = (() => {
      if (namespaces.length === 1) {
        ...
        return (
          <>
            This will permanently delete {count} replicaset{count === 1 ? "" : "s"} from namespace{" "}
      ...
      return (
        <>
          This will permanently delete {count} replicasets from {namespaces.length} namespaces. All
          associated resources will be removed.
        </>
      );
    })();
