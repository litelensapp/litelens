...
export const PodModificationTray: FC<ModificationTrayContentProps> = ({
  ...
  if (!tab.namespace) {
    return (
      <p className="text-destructive p-4 text-xs">Pod namespace is required but not provided.</p>
    );
  }
    ...
    <>
      ...
      >
        {isLoading ? (
          ...
          <p className="text-muted-foreground p-4 text-xs">No YAML available for this Pod.</p>
        ) : (
          <Textarea
            variant="yaml"
            borderRounded={false}
