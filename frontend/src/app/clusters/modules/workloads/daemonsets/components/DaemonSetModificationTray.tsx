...
import { useGetDaemonSetYAML } from "../hooks/data-access/useGetDaemonSetYAML";
import { useUpdateDaemonSetYAML } from "../hooks/data-mutation/useUpdateDaemonSetYAML";
...
export const DaemonSetModificationTray: FC<ModificationTrayContentProps> = ({
  tab,
  collapsed,
  onClose,
}) => {
  const namespace = tab.namespace || "";
  const daemonSetName = tab.name;
  ...
  } = useGetDaemonSetYAML(activeContext, namespace, daemonSetName);

  const { mutate: updateYAML, isPending: isUpdating } = useUpdateDaemonSetYAML();
  ...
  if (!tab.namespace) {
    return (
      <p className="p-4 text-xs text-destructive">
        DaemonSet namespace is required but not provided.
      </p>
    );
  }
    ...
    <>
      ...
      >
        {isLoading ? (
          ...
          <p className="p-4 text-xs text-muted-foreground">No YAML available for this DaemonSet.</p>
        ) : (
          <Textarea
            variant="yaml"
            borderRounded={false}
