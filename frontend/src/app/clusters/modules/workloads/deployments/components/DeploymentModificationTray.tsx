...
import { useGetDeploymentYAML } from "../hooks/data-access/useGetDeploymentYAML";
import { useUpdateDeploymentYAML } from "../hooks/data-mutation/useUpdateDeploymentYAML";
...
export const DeploymentModificationTray: FC<ModificationTrayContentProps> = ({
  tab,
  collapsed,
  onClose,
}) => {
  const namespace = tab.namespace || "";
  const deploymentName = tab.name;
  ...
  } = useGetDeploymentYAML(activeContext, namespace, deploymentName);

  const { mutate: updateYAML, isPending: isUpdating } = useUpdateDeploymentYAML();
  ...
  if (!tab.namespace) {
    return (
      <p className="text-destructive p-4 text-xs">
        Deployment namespace is required but not provided.
      </p>
    );
  }
      ...
      >
        {isLoading ? (
          ...
          <p className="text-muted-foreground p-4 text-xs">
            No YAML available for this Deployment.
          </p>
        ) : (
          <Textarea
            variant="yaml"
